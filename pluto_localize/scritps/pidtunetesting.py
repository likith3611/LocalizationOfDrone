...
class DroneFly():
    ...
    def calc_pid(self):
        ...
        if(current_time >= self.loop_time):
            ...
            self.pid_yaw(current_time)
            self.last_time = self.seconds


    def pid_yaw(self, delta):
    ...
    def pid_roll(self, delta):
        self.error_roll = self.wp_x - self.drone_x
        self.P_value_roll = self.kp_roll * self.error_roll
        self.D_value_roll = self.kd_roll * ( self.error_roll - self.DerivatorR) / delta
        self.DerivatorR = self.error_roll

        self.IntegratorR = self.IntegratorR + self.error_roll * delta
    ...
    def pid_pitch(self, delta):
        self.error_pitch = self.wp_z - self.drone_z
        self.P_value_pitch = self.kp_pitch * self.error_pitch
        self.D_value_pitch = self.kd_pitch * ( self.error_pitch - self.DerivatorP)/delta
        self.DerivatorP = self.error_pitch

        self.IntegratorP = self.IntegratorP + self.error_pitch * delta
    ...
    def pid_throt(self, delta):
        self.error_throt = self.wp_y - self.drone_y
        self.P_value_throt = self.kp_throt  * self.error_throt
        self.D_value_throt = self.kd_throt * ( self.error_throt - self.DerivatorT)/delta
        self.DerivatorT = self.error_throt

        self.IntegratorT = self.IntegratorT + self.error_throt*delta
